...
const statusLabels: Record<string, { label: string; color: string }> = {
  PUBLISHED: { label: "已发布", color: "bg-[#6366F1]/10 text-[#6366F1]" },
  DRAFT: { label: "草稿", color: "bg-[#C7D2FE]/60 text-[#5a6b66]" },
      ...
      <div className="flex">
          ...
          <nav className="flex-1 p-4 space-y-1">
            {[
              ...
              <Link key={item.href} href={item.href}>
                <button className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${item.href === '/admin/activities' ? 'bg-[#6366F1]/10 text-[#6366F1]' : 'text-[#5a6b66] hover:bg-[#6366F1]/5 hover:text-[#6366F1]'}`}>
        ...
        <main className="flex-1 p-6 md:p-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div><h1 className="text-2xl font-bold text-[#6366F1]">活动管理</h1><p className="text-[#8a9490] text-sm">共 {activities.length} 个活动</p></div>
          </div>

          <div className="mb-6">
            <div className="relative max-w-md">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#8a9490]" />
              <Input placeholder="搜索活动..." className="input-elegant pl-10 border-[#C7D2FE] focus:border-[#6366F1] focus:ring-[#6366F1]/20" />
            </div>
          </div>
          ...
          <Card className="card-elegant border-[#C7D2FE] shadow-sm">
            <CardContent className="p-0">
              <table className="w-full">
                ...
                <tbody>
                  {activities.map((act) => (
                    <tr key={act.id} className="border-b border-[#C7D2FE] hover:bg-[#F0F0FF]/70 transition-colors">
                      <td className="p-4">
                        <div>
                          <p className="font-medium text-sm text-[#6366F1]">{act.title}</p>
                          <p className="text-xs text-[#8a9490]">by {act.organizer}</p>
                        </div>
                      </td>
                      <td className="p-4"><Badge variant="outline" className="tag-elegant text-xs border-[#C7D2FE] text-[#5a6b66]">{typeLabels[act.type]}</Badge></td>
                      ...
                      <td className="p-4">
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-[#6366F1] hover:bg-[#6366F1]/5"><Eye className="h-4 w-4" /></Button>
                          {act.status === "DRAFT" && (
                            <Button variant="ghost" size="icon" className="btn-secondary h-8 w-8 text-[#6366F1] hover:bg-[#6366F1]/5"><CheckCircle2 className="h-4 w-4" /></Button>
                          )}
